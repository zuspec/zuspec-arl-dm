...
ExecGraphNodeUP ExecGraphBuilder::build(
...
ScheduleGraphNode *ExecGraphBuilder::processSequence(
	...
	while (node && node->getOutgoingArcs().size() <= 1) {
		parent->addChild(new ExecGraphNode(
...
ScheduleGraphNode *ExecGraphBuilder::processParallel(
	...
	parent->addChild(new ExecGraphNode(ExecGraphNodeKindE::Traverse, 0));
	...
	ExecGraphNode *prev_fork_join = 0;
	for (auto it=end_nodes.begin(); it!=end_nodes.end(); it++) {
		...
		for (auto b=b_it->second.begin(); b!=b_it->second.end(); b++) {
			...
			if (branch->children().size() > 1) {
				...
				fork_join->addChild(branch->children().at(0).release());
				delete branch;
			}
		}


		// Add in the start nodes
